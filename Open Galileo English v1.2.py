...
def imread_unicode(path):
    ...
    if ext in RAW_EXTS:
        if rawpy is None:
            raise RuntimeError(
                'To abrir archivos RAW (.nef/.raw/.dng/.arw/.cr2/...) instala the dependencia:\n'
                "pip install rawpy"
            )
        try:
            ...
            raise RuntimeError(f"Error reading RAW {path}: {e}")
...
def load_images_full_and_small(paths, align_max_side=1600):
    ...
    Optimización (sin cambiar resultados):
    - small_imgs se mantiene en uint8 porque ECC trabaja en uint8.
    - se evita construir listas innecesarias; el scale_factor_small conserva el comportamiento previo
      (proviene de la primera imagen cargada).
    """
    full_imgs = []
    small_imgs = []
    scale_small = None
...
def _pmax_stack_is_predominantly_dark(images_full, *, pix_thr=55, frac_thr=0.20, med_thr=90, max_side=420): # qué tanto negro hay en la imagen
    """
    Heurística rápida para detectar stacks con fondo predominantemente negro.

    - Convierte a escala de grises (usa 1..3 frames para robustez).
    - Reduce tamaño para acelerar.
    - Marca como "oscuro" si:
        * fracción de píxeles <= pix_thr es alta (>= frac_thr), y
        * la mediana global es baja (<= med_thr).
    """
    if not images_full:
        return False
...
def pmax_multiband_dark(images_full, levels=5, weight_temp=0.65, progress_cb=None):
    """
    PMax multi-banda con reducción de halo (selección dura por nivel + suavizado anti-halo).

    - Niveles Laplacianos (0..levels-1):
        energía = sum(abs(L)) por canal, suavizada con kernel dinámico para reducir halo.
        se elige (argmax) el frame ganador por píxel y por nivel (selección dura).
    - Nivel Residual (lvl==levels):
        NO calcula energía (evita sesgo por brillo). Copia los índices ganadores del nivel anterior
        (lvl-1), reescalados con nearest si cambia el tamaño.
    - Reconstrucción con pyrUp(dstsize=...) para tamaños impares.
    - Clipping final 0..255.

    NOTA:
    - Mantiene la firma original (weight_temp se ignora; se conserva por compatibilidad).
    - Los sliders globales RADIUS_PMAX / SMOOTH_PMAX influyen en el suavizado anti-halo (kernel base).
    """
    n = len(images_full)
    if n == 0:
        return None
...
def cleanup_black_background(
    ...
    """
    'Negro' para stacks con fondo negro.

    Problema típico: el post-procesado (CLAHE/unsharp) levanta el negro y deja
    un 'haze' o granulado visible en el fondo. Dejar el fondo más
    "clavado" a negro.

    Estrategia:
    1) Trabajar sobre un gris SUAVIZADO (blur) para que la textura del sujeto
       (celdas, bordes) no se confunda con fondo.
    2) Marcar como fondo SOLO las zonas oscuras CONECTADAS AL BORDE de la imagen
       (el fondo real).
    3) Empujar ese fondo a negro puro con un feather suave para matar halos.

    Parámetros:
    - gray_thr: umbral de "oscuro" (más alto = más agresivo con halos).
    - blur_sigma: cuánto se suaviza para ignorar textura del sujeto.
    - feather: suavizado del borde de la máscara (0 = corte duro).
    - morph_ksize: cierra pequeños agujeros en la máscara de fondo.
    """
...
def pmax_multiband(images_full, levels=5, weight_temp=0.65, progress_cb=None, force_dark=None):
    """
    PMax:
    - AUTO (force_dark=None): detecta si el stack es predominantemente oscuro y elige el algoritmo.
    - FORZADO (force_dark=True/False): usa directamente el algoritmo de oscuro / claro.

    force_dark:
      - None  -> auto detección (comportamiento original)
      - True  -> forzar algoritmo para fondo negro (oscuro)
      - False -> forzar algoritmo para fondo claro (blanco)
    """
...
def focus_weighted_average(images_full, weight_temp=2.0, progress_cb=None, use_dark_bg=False):
    """
    Fusión por promedio ponderado (Promedio Ponderado).

    Objetivo: mantener el look “promedio” pero reducir halos.
    - Fondo claro: pesos continuos (softmax) + fusión multi-banda (pirámide Laplaciana).
    - Fondo negro: pesos más “duros” + sesgo a elegir píxeles más oscuros en zonas con poco foco
      (reduce el levantado del negro y el halo alrededor del sujeto).
...
def focus_depth_map_fusion(images_full, smooth_ksize=9, smooth_sigma=2.0, progress_cb=None, use_dark_bg=False):
    ...
    - Fondo claro: índice ganador por píxel (máximo foco) + suavizado del mapa de índices.
    - Fondo negro: aplica la MISMA idea anti-halo de PMax/Promedio Ponderado:
        * Detecta zonas de baja textura / baja confianza (poco foco o empate entre frames)
        * En esas zonas sesga hacia el píxel MÁS OSCURO entre los frames (evita el "levantado"
          del negro y los halos alrededor del sujeto).
...
def apply_wavelet_denoising(img_bgr, wavelet="db2", level=2):
    ...
    if pywt is None:
        raise RuntimeError("To usar 'Denoising via Wavelets' instala: pip install PyWavelets")
...
def apply_bm3d_denoising(
    ...
    def _bm3d_run(ch01):
        ...
        raise RuntimeError(f"Error running BM3D: {last}")
    ...
    for c in range(channels_total):
        ...
        if progress_cb:
            progress_cb(f"BM3D {label} — preparing…", c, channels_total, None)
...
def _prepare_aligned_standard(paths, progress_cb=None):
    """
    Carga + alinea una serie usando el mismo alineamiento estándar (ECC multiescala)
    que usan PMax / Promedio Ponderado / Mapa de profundidad.

    Se usa para reutilizar el mismo stack alineado entre métodos cuando se seleccionan varios,
    acelerando el flujo sin cambiar el resultado.
    """
    full_imgs, small_imgs, scale_small = load_images_full_and_small(paths)
    if len(full_imgs) < 2:
        raise RuntimeError('Not enough images could be loaded.')
...
def stack_paths(paths, progress_cb=None, force_dark=None, prepared_aligned_full=None, _emit_cached_stages=False):
    ...
    if result_raw is None:
        raise RuntimeError('Failed the stacking.')
...
def stack_paths_weighted(paths, progress_cb=None, force_dark=None, prepared_aligned_full=None, _emit_cached_stages=False):
    """
    Pipeline alternativo: Promedio Ponderado.
    Alinea igual que PMax, fusiona con focus_weighted_average y luego aplica
    el mismo post-procesado (clarity-like + manejo de negro) que PMax.

    - Detecta fondo oscuro con _pmax_stack_is_predominantly_dark() (o override por switch),
      y usa una fusión distinta en Promedio Ponderado para reducir halo en fondos negros.
    """
    aligned_full = prepared_aligned_full
    if aligned_full is None:
        full_imgs, small_imgs, scale_small = load_images_full_and_small(paths)
    ...
    if result is None:
        raise RuntimeError('Failed the stacking (average weighted).')
...
def stack_paths_depth(paths, progress_cb=None, force_dark=None, prepared_aligned_full=None, _emit_cached_stages=False):
    ...
    Luego aplica el mismo post-procesado (clarity-like + manejo de negro)
    que PMax, usando la misma lógica de detección de fondo oscuro (o override por switch).
    """
    aligned_full = prepared_aligned_full
    if aligned_full is None:
        full_imgs, small_imgs, scale_small = load_images_full_and_small(paths)
    ...
    if result is None:
        raise RuntimeError('Failed the stacking (map of depth).')
...
class FocusStackerApp(tk.Tk):
    def __init__(self):
        ...
        def _startup():
            ...
            self.title(
                "Open Galileo v1.2 / Programa desarrollado por Brandon Antonio Segura Torres / @micro.cosmonauta / antoniosegura3d@gmail.com")
            ...
            # Si el usuario toca el switch Blanco/Negro mientras hay un lote de carpetas cargado,
            # se fuerza ese modo para TODOS los apilados de focus tracking del batch y se desactiva la detección automática.
            self._batch_force_bg_override = False
...
# ---------- botones con iconos ----------
        ...
        ToolTip(self.btn_add, 'Add an image series to the stack')
        ...
        tk.Label(
            label_frame,
            text='Focus stacking (focus tracking)',
            bg=UI_BG,
            fg=TXT_FG,
            font=("", 10, "bold")
        ...
        # Si el usuario hace click en el switch, ejecutamos un callback (importante para batch de carpetas)
        try:
            self.rb_pmax_bg_white.config(command=self._on_pmax_bg_switch_clicked)
            self.rb_pmax_bg_black.config(command=self._on_pmax_bg_switch_clicked)
        except Exception:
            pass

        # Tooltip del switch (explicación breve)
        tip_pmax_bg = 'Detecta automatically si the serie es of background claro u oscuro to aplicar the algoritmo especializado. Cambia este switch to forzar the mode opuesto (White/Black)'
        ...
        ToolTip(
            ...
            'Applies the selected filters one after another (in the numbered order),\naccumulating their effects on the image. This can improve the result,\nbut it increases processing time.'
        ...
        try:
            ...
            tip_default = 'Disable all checkboxes from the other method to enable these.'
            for _w in (
...
# Aplicar bloqueo inicial entre grupos
        ...
        ToolTip(
            self.btn_process,
            'Inicia the processing and stacking of images.'
        ...
        self.status = tk.StringVar(value='No images cargadas.')
        tk.Label(
            progress_container,
        ...
        ToolTip(self.btn_save, 'Select the path for automatic file saving')
        ...
        ToolTip(self.btn_clear, 'Clear added image series or folders')
        ...
        DisabledToolTip(
            ...
            'To usar estos sliders, habilite the method "PMax" (marque the casilla PMax).'
        )
        DisabledToolTip(
        ...
        DisabledToolTip(
            ...
            'To usar estos sliders, habilite the method "Average weighted" (marque the casilla Average weighted).'
        )
        DisabledToolTip(
        ...
        DisabledToolTip(
            ...
            'To usar estos sliders, habilite the method "Map of depth" (marque the casilla Map of depth).'
        )
        DisabledToolTip(
        ...
        _tt_mean = 'To usar estos sliders, habilite the method "Average (mean stacking)" (marque the casilla Average).'
        ...
        _tt_median = 'To usar estos sliders, habilite the method "Mediana (median stacking)" (marque the casilla Mediana).'
        ...
        _tt_sum = 'To usar estos sliders, habilite the method "Suma (sum stacking)" (marque the casilla Suma).'
        ...
        _tt_sigma = 'To usar estos sliders, habilite the method "Sigma-clipping" (marque the casilla Sigma-clipping).'
        ...
        _tt_bm3d = 'To usar estos sliders, habilite the filter "BM3D" (marque the casilla BM3D).'
        ...
        _tt_bil = 'To usar estos sliders, habilite the filter "Bilateral" (marque the casilla Bilateral).'
        ...
        _tt_wav = 'To usar este slider, habilite the filter "Wavelet Denoising" (marque the casilla Wavelet).'
        ...
        _tt_nlm = 'To usar estos sliders, habilite the filter "Non-Local Means" (marque the casilla NLM).'
        ...
        _tt_four = 'To usar estos sliders, habilite the filter "Filtrado en Fourier" (marque the casilla Filtrado en Fourier).'
        ...
        _tt_n2v = 'To usar estos sliders, habilite the filter "Noise2Noise / Noise2Void" (marque the casilla Noise2Noise / Noise2Void).'
        ...
        _tt_med = 'To usar este slider, habilite the filter "Filter Mediana" (marque the casilla Filter Mediana).'
        ...
        _tt_gau = 'To usar estos sliders, habilite the filter "Filter Gaussiano" (marque the casilla Filter Gaussiano).'
        ...
        tk.Label(work, text='Image fuente (en uso):',
    ...
    def add_images(self):
        ...
        # Si veníamos de un batch forzado, al cargar una SERIE volvemos al comportamiento normal
        try:
            self._batch_force_bg_override = False
        except Exception:
            pass
    ...
    def _on_pmax_bg_switch_clicked(self):
        """Callback cuando el usuario hace click en el switch Blanco/Negro.

        - En serie (imágenes sueltas): el switch ya actúa como override para los 3 métodos.
        - En batch (lote de carpetas): si el usuario lo toca, se fuerza el modo para TODO el lote
          y se desactiva la detección automática por carpeta para focus tracking.
        """
        try:
            in_batch = bool(getattr(self, "folder_paths", []))
        except Exception:
            in_batch = False

        # Solo activar override de batch si realmente hay carpetas cargadas
        if in_batch:
            try:
                self._batch_force_bg_override = True
            except Exception:
                pass

            try:
                mode = (self.pmax_bg_mode_var.get() or "white").strip().lower()
            except Exception:
                mode = "white"

            if mode == "black":
                algo_humano = 'BACKGROUND BLACK (oscuro)'
                algo_tecnico = "force_dark=True / use_dark_bg=True"
            else:
                algo_humano = 'BACKGROUND CLARO (white)'
                algo_tecnico = "force_dark=False / use_dark_bg=False"

            msg = (
                f"The entire batch of folders will be processed with the algorithm: {algo_humano}.\\n\\n"
                f"Mode selected manually: {mode.upper()}\\n"
                f"Technical detail: {algo_tecnico}\\n\\n"
                'From this point on, Open Galileo will NOT perform automatic detection '
                'of background (claro/black) via folder to the methods of focus tracking '
                '(Pyramid Max + Post, Average Weighted and Map of depth).'
            )

            try:
                messagebox.showinfo('Forced background mode (Batch)', msg)
            except Exception:
                pass

        # refrescar visualmente (por si hay estilos/estados)
        try:
            self._update_slider_state()
        except Exception:
            pass
    ...
    def _update_slider_state(self):
        ...
        try:
            has_any = bool(getattr(self, "image_paths", [])) or bool(getattr(self, "folder_paths", []))
            enable_switch = (self.use_pmax.get() or self.use_weighted.get() or self.use_depth.get()) and has_any
    ...
    def _set_chk_enabled(self, chk, enabled: bool, exclusivity_tip: str = None):
        """Habilita/Deshabilita un Checkbutton y deja marcado si fue por exclusividad (para tooltips)."""
        if chk is None:
            return
        try:
            chk.config(
    ...
    def _update_focus_noise_exclusivity(self):
        ...
        tip_blocked_by_focus = 'Disable all "Focus stacking (focus tracking)" checkboxes to enable these.'
    ...
    def _on_pmax_black_bg_thr_change(self, val):
        """Slider 'Umbral fondo negro' -> PMAX_BLACK_BG_GRAY_THR."""
    ...
    def _on_weighted_black_bg_thr_change(self, val):
        """Slider 'Umbral fondo negro' (Promedio ponderado) -> WEIGHTED_BLACK_BG_GRAY_THR (+ STRONG)."""
    ...
    def _on_depth_black_bg_thr_change(self, val):
        """Slider 'Umbral fondo negro' (Mapa de profundidad) -> DEPTH_BLACK_BG_GRAY_THR (+ STRONG)."""
    ...
    def add_folders(self):
        ...
        """
        base_dir = filedialog.askdirectory(title='Select a folder contains folders with images')
        if not base_dir:
            return
        ...
        if not subfolders:
            messagebox.showwarning('No subfolders', 'The selected folder has no subfolders.')
            return
        ...
        # Por defecto, en batch se mantiene la detección automática por carpeta,
        # hasta que el usuario toque el switch Blanco/Negro.
        try:
            self._batch_force_bg_override = False
        except Exception:
            pass
        ...
        self._set_progress(0, f"{len(valid_folders)} folders ready to process.", None, None)
        self.btn_save.config(state=tk.NORMAL)
        self.result = None
        self._clear_previews()
    ...
    def _get_images_for_folder(self, folder):
        """Devuelve imágenes válidas de una carpeta, filtrando las excluidas por el usuario.
        Nota: esto NO borra archivos del disco; solo los excluye del procesamiento."""
        imgs = []
        try:
            imgs = list_images_in_folder(folder)
        except Exception:
    ...
    def _lb_restore_folder_list(self, keep_selection=0):
        """Vuelve a la vista de carpetas (batch)."""
        try:
            self._lb_mode = "folders"
            self._lb_detail_folder = None
            self._lb_detail_img_paths = []
    ...
    def _lb_show_folder_detail(self, folder_path, keep_selection=1):
        """Despliega una carpeta mostrando sus imágenes."""
    ...
    def _on_listbox_click(self, event=None):
        """Click: en batch, despliega/colapsa carpetas. En serie, solo enfoca (preview lo maneja el Select)."""
    ...
    def _on_listbox_select(self, event=None):
        """Selección: si es imagen, abrirla en 'Imagen fuente (en uso)'."""
        if getattr(self, "_is_processing", False):
            return
    ...
    def _on_listbox_delete(self, event=None):
        """Tecla Supr: elimina de la lista (solo excluye del procesamiento; NO borra del disco)."""
        if getattr(self, "_is_processing", False):
            return "break"
            ...
            if getattr(self, "_lb_mode", "folders") == "folders":
                ...
                try:
                    if self.folder_paths:
                        ...
                        self._lb_select_index(new_idx)
                        self._set_progress(0, f"{len(self.folder_paths)} folders ready to process.", None, None)
                    else:
                        self._lb_mode = "empty"
                        self._set_progress(0, 'Empty list.', None, None)
                        self._clear_previews()
    ...
    def _update_autosave_label(self):
        if self.auto_save_dir:
            self.auto_save_label_var.set(f"Saved automatic:\\n{self.auto_save_dir}")
        else:
    ...
    def show_about(self):
        about_text = (
            'Open Galileo – Advanced scientific and macro image stacking.\n\n'
            'Open Galileo is a free, open-source program developed by Brandon Antonio Segura Torres @micro.cosmonauta.\n'
            ...
            'open science and democratic access to advanced tools.\n'
            'It is free, with no restrictions, and intended to grow with the community.\n'
            'Open Galileo is and will always be free.\n'
            'If you find this program useful and want to support the time, effort, and research\n'
            ...
            "(alias): opengalileo\n"
            'Mercado Pago CVU: 0000003100022518896098\n'
            'Rest of the world via PayPal: antoniovangritte@gmail.com'
        )
    ...
    def _load_luna_gif(self, gif_path, size=(30, 30)):
        ...
        try:
            ...
            print(f"[WARN] Could not load luna.gif: {e}")
            self._luna_frames = []
            self._luna_durations = []
    ...
    def _tick_processing_timer(self):
        ...
        try:
            self.timer_var.set(f"Processing time: {mm:02d}:{ss:02d}")
        except Exception:
            pass
    ...
    def ensure_calibration_loaded(self):
        ...
        info = (
            ...
            ' - Dark: black frame, same exposure/ISO as the sky images.\n'
            ' - Flat: uniform sky or flat panel, same optics and focus.\n'
            ' - Bias: minimum exposure, lens capped (sensor readout offset).\n'
        ...
        make_row(
            body,
            "dark",
            'Master Dark (black frame)',
            'Sensor thermal noise. Same exposure/ISO as the light frames, lens/cap covering the light path.',
        )
        make_row(
            body,
            "flat",
            'Master Flat (flat field)',
            'Corrects vignetting and dust spots. Uniform sky / illuminated panel, same position and focus as the light frames.',
        )
        make_row(
            body,
            "bias",
            "Master Bias (offset)",
            'Minimum exposure frames, lens capped. Corrects the sensor readout offset.',
        )
        ...
        def load_master(path, label):
            ...
            if img is None:
                messagebox.showerror("Error", f"Could not load master {label}:\\n{path}")
                return None
        ...
        if not any(x is not None for x in (self.master_dark, self.master_flat, self.master_bias)):
            messagebox.showwarning(
                ...
                'The calibration D/F/B no will apply.'
            )
            return False
            ...
            try:
                def ui_cb(stage, current, total, preview):
                    ...
                    if stage == "load":
                        ...
                        self._set_progress(pct, f"Map of foco {current}/{total}...", preview, None)
                    elif stage == "pyramid":
                        pct = self._stage_progress_mapper(65, 20, current, total)
                        self._set_progress(pct, f"Building pyramid {current}/{total}...", None, preview)
                    elif stage == "post":
                        self._set_progress(85, 'Clarity-like post-processing...', None, preview)
                ...
                self.result = result
                self._set_progress(100, 'Done (PMax). Saving automatically...', None, result)
                ...
                if self.auto_save_dir:
                    ...
                    self.status.set(f"Saved automatic en: {out_path}")
                else:
                    self.status.set("Processing complete. Select folder with 'Save JPG'.")
    ...
    def process_stack_weighted(self):
        ...
        self._disable_all_buttons(True)
        self._set_progress(0, 'Processing stack (Average weighted)...', None, None)
        ...
        def worker():
            try:
                ...
                self.result = result
                self._set_progress(100, 'Done (Average). Saving automatically...', None, result)

                if self.auto_save_dir:
                    base = "resultado"
                    out_path = self._unique_out_path(self.auto_save_dir, base)
                    self._save_image(out_path, result)
                    self.status.set(f"Saved automatic (Average) en: {out_path}")

                else:
                    self.status.set("Processing complete (Average). Select folder with 'Save JPG'.")
                self.btn_save.config(state=tk.NORMAL)
    ...
    def apilar_imagenes(self):
        ...
        if not self.image_paths:
            messagebox.showwarning("Sin elementos", 'Add images or folders.')
            return
        ...
        if not methods and not filters_only_mode:
            messagebox.showwarning(
                'No method selected',
                'Select to the least a method of stacking or enable to the least a filter of noise.'
            )
            return
        ...
        if filters_only_mode:
            self._disable_all_buttons(True)
            self._set_progress(0, 'Aplicando filters a todas the images...', None, None)
            ...
            def worker_filters():
                try:
                    ...
                    self._set_progress(0, 'Error to the aplicar filters a the images.', None, None)
                finally:
                    def _finish_ui():
                        try:
                            self._is_processing = False
            ...
            try:
                ...
                for idx, m in enumerate(methods, start=1):
                    ...
                    if m == "pmax":
                        ...
                        def ui_cb(stage, current, total, preview):
                            ...
                            if stage == "load":
                                ...
                                self._set_progress(pct, f"{prefix} Map of foco {current}/{total}.", preview, None)
                            elif stage == "pyramid":
                                pct = self._stage_progress_mapper(60, 20, current, total)
                                self._set_progress(pct, f"{prefix} Building pyramid {current}/{total}.", None, preview)
                            elif stage == "post":
                        ...
                        def ui_cb(stage, current, total, preview):
                            if stage == "load":
                                ...
                                pct = self._stage_progress_mapper(45, 20, current, total)
                                self._set_progress(pct, f"{prefix} Calculando map of foco {current}/{total}...",
                                                   preview, None)
                            elif stage == "fusionD":
                                pct = self._stage_progress_mapper(70, 25, current, total)
                        ...
                        raise RuntimeError(f"Method desconocido: {m}")
                    ...
                    if self.auto_save_dir:
                        ...
                        out_path = self._unique_out_path(self.auto_save_dir, base)
                        self._save_image(out_path, result)
                        self.status.set(f"{prefix} Saved automatic ({method_tag}) en: {out_path}")
                    else:
                        self.status.set(f"{prefix} Processing complete ({method_tag}).")

                if last_result is not None:
                    self._set_progress(100, 'Stacks completed.', None, last_result,
                                       active={"type": "clear_highlight"})
                else:
                    self._set_progress(0, 'No generated no result.', None, None,
                                       active={"type": "clear_highlight"})
                ...
                messagebox.showerror("Error", str(e))
                self._set_progress(0, 'Error during the process of stacking.', None, None)
            finally:
                def _finish_ui():
                    try:
                        self._is_processing = False
    ...
    def process_stack_depth(self):
        ...
        self._disable_all_buttons(True)
        self._set_progress(0, 'Processing stack (Map of depth)...', None, None)
            ...
            try:
                def ui_cb(stage, current, total, preview):
                    ...
                    if stage == "load":
                        ...
                        self._set_progress(pct, f"Calculando map of foco {current}/{total}...", preview, None)
                    elif stage == "refineD":
                ...
                self.result = result
                self._set_progress(100, 'Done (Map depth). Saving automatically...', None, result)

                if self.auto_save_dir:
                    base = "resultado"
                    out_path = self._unique_out_path(self.auto_save_dir, base)
                    self._save_image(out_path, result)
                    self.status.set(f"Saved automatic (Map depth) en: {out_path}")

                else:
                    self.status.set("Processing complete (Map depth). Select folder with 'Save JPG'.")
                self.btn_save.config(state=tk.NORMAL)
    ...
    def process_folders(self):
        if len(self.folder_paths) == 0:
            messagebox.showwarning('Faltan folders', 'Add at least a folder.')
            return
        ...
        if not out_root:
            out_root = filedialog.askdirectory(title='Select folder destino to results')
            if not out_root:
                return
        ...
        if filters_only_mode:
            ...
            if not selected_tags:
                messagebox.showwarning(
                    "Sin filtros",
                    'Enable to the least a filter of noise to process the folders.'
                )
                return
            ...
            if total_jobs <= 0:
                messagebox.showwarning("Sin trabajo", 'No images/filters to process.')
                return

            self._disable_all_buttons(True)
            self._set_progress(0, 'Aplicando filters a images en folders.', None, None)
            ...
            def worker():
                try:
                    ...
                    for f_idx, (folder, img_paths) in enumerate(folder_to_imgs, start=1):
                        ...
                        if not img_paths:
                            self._set_progress(
                                ...
                                f"[{f_idx}/{total_folders}] Folder no images: {folder_name}",
                                None,
                                None
                            )
                            continue
                        ...
                        for p in img_paths:
                            ...
                            if img is None:
                                ...
                                self._set_progress(
                                    100.0 * done / total_jobs,
                                    f"[{f_idx}/{total_folders}] Error reading: {os.path.basename(p)}",
                                    None,
                                    None
                                )
                                continue
                    ...
                    if last_result is not None:
                        self._set_progress(100, 'Filter processing in folders finished.', None, last_result)
                    else:
                        self._set_progress(100,
                                           'Filter processing in folders finished (no valid results).',
                                           None, None)
                    ...
                    if not err_txt:
                        err_txt = repr(e)
                    msg = (
                        ' produced a error during the processing via batches (batch). '
                        'Posible causa: a image of the secuencia no tiene the misma orientation/rotation the others '
                        '(via example, a photo rotated 90° or with metadata EXIF of orientation different). '
                        'Tip: check the folder and make sure all images are oriented the same before stacking.\n\n'
                        f"Detalle technical:\\n{err_txt}"
                    )
                    self.after(0, lambda m=msg: messagebox.showerror('Error of batch', m))
                    self._set_progress(0, 'Error in batch process (Filters).', None, None)
        ...
        if not methods:
            messagebox.showwarning('No method selected', 'Select at least one stacking method.')
            return
        ...
        # Si el usuario tocó el switch Blanco/Negro con un lote de carpetas cargado,
        # se fuerza ese modo para TODOS los apilados de focus tracking del batch.
        force_dark_batch = None
        try:
            if getattr(self, "_batch_force_bg_override", False):
                force_dark_batch = ((self.pmax_bg_mode_var.get() or "white").strip().lower() == "black")
        except Exception:
            force_dark_batch = None
        ...
        def worker():
            try:
                ...
                for f_idx, folder in enumerate(folders, start=1):
                    ...
                    if len(img_paths) < 2:
                        ...
                        for _ in methods:
                            ...
                            self._set_progress(
                                pct,
                                f"[{f_idx}/{total_folders}] Folder no enough images: {folder_name}",
                                None,
                                None
                            )
                        continue
                    ...
                    self._set_progress(
                        pct_folder,
                        f"[{f_idx}/{total_folders}] Preparando {folder_name} ({len(img_paths)} images)...",
                        None,
                        None,
                        ...
                        if m == "pmax":
                            ...
                            def ui_cb(stage, current, total, preview):
                                if stage == "load":
                                    ...
                                    self._set_progress(
                                        pct,
                                        f"[{f_idx}/{total_folders}] ({method_tag}) Map of foco {current}/{total}.",
                                        preview,
                                        None
                                    )
                                elif stage == "pyramid":
                            ...
                            result = stack_paths(img_paths, progress_cb=ui_cb, force_dark=force_dark_batch)
                            ...
                            result = stack_paths_weighted(img_paths, progress_cb=ui_cb, force_dark=force_dark_batch)
                            ...
                            def ui_cb(stage, current, total, preview):
                                if stage == "load":
                                    ...
                                    self._set_progress(
                                        pct,
                                        f"[{f_idx}/{total_folders}] ({method_tag}) Fusing.",
                                        None,
                                        preview
                                    )

                            result = stack_paths_depth(img_paths, progress_cb=ui_cb, force_dark=force_dark_batch)
                ...
                self.after(0, lambda m=msg: messagebox.showerror('Error of batch', m))
                self._set_progress(0, 'Error in batch process.', None, None,
                                   active={"type": "restore_listbox"})
            finally:
                def _finish_ui():
                    try:
    ...
    def save_result(self):
        ...
        if self.result is not None:
            ...
            self.status.set('Folder selected to saved automatic.')
