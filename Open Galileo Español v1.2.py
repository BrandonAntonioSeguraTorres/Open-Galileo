...
# ---------- botones con iconos ----------
        ...
        tip_pmax_bg = "Detecta automáticamente si la serie es de fondo claro u oscuro para aplicar el algoritmo especializado.\n Cambia este switch para forzar el modo opuesto (Blanco/Negro)"
        ...
        try:
            ...
            tip_default = "Deshabilite todas las casillas del otro método para habilitar estas."
            for _w in (
...
# Aplicar bloqueo inicial entre grupos
        ...
        ToolTip(self.btn_clear, "Limpiar serie de imágenes o carpetas agregadas")
    ...
    def _on_pmax_bg_switch_clicked(self):
        ...
        if in_batch:
            ...
            if mode == "black":
                algo_humano = "FONDO NEGRO (oscuro)"
                algo_tecnico = "force_dark=True / use_dark_bg=True"
            else:
                algo_humano = "FONDO CLARO (blanco)"
                algo_tecnico = "force_dark=False / use_dark_bg=False"

            msg = (
                f"Todo el lote de carpetas será procesado con el algoritmo: {algo_humano}.\n\n"
                f"Modo seleccionado manualmente: {mode.upper()}\n"
                f"Detalle técnico: {algo_tecnico}\n\n"
                "A partir de este momento, Open Galileo NO realizará la detección automática "
                "de fondo (claro/negro) por carpeta para los métodos de focus tracking "
                "(Pirámide Max + Post, Promedio Ponderado y Mapa de profundidad)."
            )

            try:
                messagebox.showinfo("Modo de fondo forzado (Batch)", msg)
            except Exception:
                pass
    ...
    def _update_focus_noise_exclusivity(self):
        ...
        tip_blocked_by_focus = 'Deshabilite todas las casillas de "Apilamiento con focus tracking" para habilitar estas.'
        tip_blocked_by_integ = 'Deshabilite todas las casillas de "Apilado por integración" para habilitar estas.'
        tip_blocked_by_noise = 'Deshabilite todas las casillas de "Filtros de ruido" para habilitar estas.'
    ...
    def _lb_show_folder_detail(self, folder_path, keep_selection=1):
        ...
        try:
            self.listbox.delete(0, tk.END)
            self.listbox.insert(tk.END, f"[CARPETA] {folder_name}")
    ...
    def _on_listbox_delete(self, event=None):
        ...
        if getattr(self, "image_paths", None) and not getattr(self, "folder_paths", None):
            ...
            if self.image_paths:
                ...
                try:
                    self._set_progress(0, f"{len(self.image_paths)} imágenes seleccionadas.", None, None)
                except Exception:
                    pass
                try:
                    self._lb_mode = "images"
            ...
            if getattr(self, "_lb_mode", "folders") == "folders":
                ...
                try:
                    if self.folder_paths:
                        ...
                        self._lb_select_index(new_idx)
                        self._set_progress(0, f"{len(self.folder_paths)} carpetas listas para procesar.", None, None)
                    else:
                        self._lb_mode = "empty"
                        self._set_progress(0, "Lista vacía.", None, None)
                        self._clear_previews()
    ...
    def _apply_ui_action(self, action):
        ...
        if t == "folder_start":
            folder_name = action.get("folder_name") or "carpeta"
        ...
        try:
            ...
            if getattr(self, "folder_paths", None):
                for folder in self.folder_paths:
                    self.listbox.insert(tk.END, f"[CARPETA] {os.path.basename(folder)}")
            else:
                    ...
                    if m == "pmax":
                        ...
                        def ui_cb(stage, current, total, preview):
                            ...
                            if stage == "load":
                                ...
                                self._set_progress(pct, f"{prefix} Fusionando pirámide {current}/{total}.", None, preview)
                            elif stage == "post":
                ...
                try:
                    ...
                    if last_result is not None:
                        ...
                        self._set_progress(100,
                                           "Procesamiento de filtros en carpetas terminado (sin resultados válidos).",
                                           None, None)
                    ...
                    if not err_txt:
                        err_txt = repr(e)
                    msg = (
                        "Se produjo un error durante el procesamiento por lotes (batch).\n\n"
                        "Posible causa: una imagen de la secuencia no tiene la misma orientación/rotación que las demás "
                        "(por ejemplo, una foto rotada 90° o con metadatos EXIF de orientación distintos).\n\n"
                        "Sugerencia: revisa la carpeta y asegura que todas las imágenes estén orientadas igual antes de apilar.\n\n"
                        f"Detalle técnico:\n{err_txt}"
                    )
                    self.after(0, lambda m=msg: messagebox.showerror("Error de batch", m))
        ...
        def worker():
            try:
                ...
                for f_idx, folder in enumerate(folders, start=1):
                    ...
                    self._set_progress(
                        pct_folder,
                        f"[{f_idx}/{total_folders}] Preparando {folder_name} ({len(img_paths)} imágenes)...",
                        None,
                        None,
